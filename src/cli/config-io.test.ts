...
describe('config-io', () => {
  ...
  function writePackageJson(dir: string): void {
    ...
    writeFileSync(
      ...
      JSON.stringify({ name: 'oh-my-opencode-air-pwsh' }),
    );
  }
  ...
  test('addPluginToOpenCodeConfig adds plugin and removes duplicates', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({ plugin: ['other', 'oh-my-opencode-air-pwsh@1.0.0'] }),
    );
    process.argv[1] = '';
    ...
    expect(saved.plugin).toContain('oh-my-opencode-air-pwsh');
    expect(saved.plugin).not.toContain('oh-my-opencode-air-pwsh@1.0.0');
  ...
  test('addPluginToOpenCodeConfig stores package name for bunx temp paths', async () => {
    ...
    const packageRoot = join(
      tmpDir,
      'bunx-1000-oh-my-opencode-air-pwsh@latest',
      'node_modules',
    ...
    expect(result.success).toBe(true);
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['oh-my-opencode-air-pwsh']);
  });
  ...
  test('addPluginToOpenCodeConfig preserves non-string plugin entries when refreshing', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other-plugin', objectPlugin, 'oh-my-opencode-air-pwsh@1.0.0'],
      }),
    );
  ...
  test('addPluginToOpenCodeConfig removes tuple plugin entries', async () => {
    ...
    writeFileSync(
      configPath,
      JSON.stringify({
        plugin: ['other', ['oh-my-opencode-air-pwsh', { enabled: true }]],
      }),
    );
    process.argv[1] = '';
    ...
    const saved = JSON.parse(readFileSync(configPath, 'utf-8'));
    expect(saved.plugin).toEqual(['other', 'oh-my-opencode-air-pwsh']);
  });
  ...
  test('addPluginToOpenCodeTuiConfig does not bypass OPENCODE_TUI_CONFIG for existing default config', async () => {
    ...
    expect(custom.plugin).toEqual(['oh-my-opencode-air-pwsh']);
